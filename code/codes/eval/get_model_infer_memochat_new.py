...
# ==== TẮT FILE‐SYSTEM MONITOR ====
# Biến này sẽ disable hoàn toàn cơ chế 
# check disk usage của raylet.
os.environ["RAY_enable_file_system_monitor"] = "false"

# ==== CHỈ HIỂN THỊ ERROR TRỞ LÊN ====
...
def get_model_answers(model_path, num_gpus, local_check, load_in_8bit, ques_jsons, prompts):
    ...
    output_data = []
    for d in _tqdm(ques_jsons, desc="Processing questions", unit="q"):
        new_d = d
...
def run_eval(model_path, num_gpus, local_check, load_in_8bit, question_file, ray_num_gpus, answer_file, prompt_path):
    ...
    ans_handles = []
    for i in _tqdm(range(0, len(ques_jsons), chunk_size),
                    desc="Dispatching chunks", unit="chunk"):
    ...
    ans_jsons = []
    for ans_handle in _tqdm(ans_handles, desc="Collecting results", unit="handle"):
